...
(define (get-required/internal path-string collector-class%)
  (define annotations (new collector-class%))
...
(define (module->typed-required/internal path-string collector%)
  ...
  (class (annotations-mixin object%)
    ...
    (define/override (syncheck:add-jump-to-definition source-obj start end id filename submods)
      (when ((current-path-filter) filename)
        (set-add! paths filename)))))

(define (module->typed-files path-string)
  (module->typed-required/internal path-string path-collector%))
